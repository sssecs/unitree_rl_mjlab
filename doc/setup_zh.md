...
重放映射后的 EgoDex 测试集双腕位置／姿态和肩高轨迹（零底盘速度）：
`./tools/play_best_ground_model.sh --viewer native --wrist-mode egodex`。
...
动态抬起段可高于该范围。
`egodex` 强制每个回合采样本轮训练所用的映射后测试集片段，保留 30 Hz
插值、起始平滑接近与片段末端保持。手部姿态是绝对目标：ARKit 手部旋转
经固定肩膀地面坐标系映射到 G1 对应固定坐标系，不会按片段起点或机器人
初始腕部姿态重新对齐。它不是独立评测。
